...
double normalize(double value, double min, double max)
{
    ...
    // Map the scaled value to the range [0, 255]
    return scaled * 255.0;
}
...
void	lighting2(t_scene *scene, t_ray *ray)
{
	...
	if (ray->distance >= 0)
	{
		...
		if (dot > 0)
		{
        	...
        	// ray->ray_colour.r = normalize(amb.r + diffuse.r + specular.r, 0, 255);
        	// ray->ray_colour.g = normalize(amb.g + diffuse.g + specular.g, 0, 255);
        	// ray->ray_colour.b = normalize(amb.b + diffuse.b + specular.b, 0, 255);
			// if ((amb.r + diffuse.r + specular.r) < 0 || (amb.r + diffuse.r + specular.r) > 255)
			// 	printf("%d\n", amb.r + diffuse.r + specular.r);
			// if ((amb.g + diffuse.g + specular.g) < 0 || (amb.g + diffuse.g + specular.g) > 255)
			// 	printf("%d\n", amb.g + diffuse.g + specular.g);
			// if ((amb.b + diffuse.b + specular.b) < 0 || (amb.b + diffuse.b + specular.b) > 255)
			// 	printf("%d\n", amb.b + diffuse.b + specular.b);
		}
		else
			ray->ray_colour = amb;
	}
